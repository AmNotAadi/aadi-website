...
function cropToTexture(img, sx, sy, sw, sh) {
  ...
  // Use imageSmoothingEnabled for pixelated look on crop too
...
export default function MinecraftCursor({
  // Use Minotar API which supports CORS via anonymous
  ...
  useEffect(() => {
    ...
    img.onerror = () => {
      ...
      // Fallback cube so cursor still works even if minotar.net fails in prod
    ...
    // Bust cache in case of proxy issues
    img.src = skinUrl + '?cachebust=' + Date.now()
    ...
    const tick = (now) => {
      ...
      if (canvas.parentElement) {
        canvas.parentElement.style.transform = `translate3d(${x}px, ${y}px, 0)`
      }
